...
const CategorySelector: React.FC<CategorySelectorProps> = ({ onCategoriaChange, onSubcategoriaChange }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const cate = await axios.get("http://localhost/php-loja-back/get-categorias.php");
        setCategorias(cate.data);

        const sub = await axios.get("http://localhost/php-loja-back/get-subcategorias.php");
